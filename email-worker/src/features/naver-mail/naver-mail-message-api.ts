...
import { markRemoteNaverMailMessageRead } from './naver-mail-read-state'
import { NaverMailAccountStore, NaverMailStoreError } from './naver-mail-store'
import type { PublicNaverMailAccount } from './naver-mail-types'
import type { Env, SessionUser } from '../../app/types'

const MAX_ATTACHMENT_BYTES = 5 * 1024 * 1024

type NaverMailMessageRow = {
  id: string
  account_id: string
  imap_uid: number
  uid_validity: number
  ...
  account_status: PublicNaverMailAccount['status']
}
...
function decodeCursor(value: string | null): { date: number; id: string } | null {
  ...
  try {
    ...
    throw new NaverMailStoreError(400, 'NAVER 邮箱邮件分页游标无效。')
  }
}

function publicMessage(row: NaverMailMessageRow) {
  return {
    id: row.id,
    account: {
      id: row.account_id,
...
export async function listNaverMailMessages(
  ...
  try {
    ...
    if (accountId && !await new NaverMailAccountStore(env, user.id).publicAccount(accountId)) {
      throw new NaverMailStoreError(404, 'NAVER 邮箱账号不存在。')
    }
    ...
    const { results } = await env.DB.prepare(
      `SELECT m.id, m.account_id, m.imap_uid, m.uid_validity, m.sender_name,
         ...
         FROM naver_mail_messages m
         JOIN naver_mail_accounts a ON a.id = m.account_id
        WHERE ${conditions.join(' AND ')}
        ORDER BY m.internal_date DESC, m.id DESC LIMIT ?`,
    ).bind(...bindings, limit + 1).all<NaverMailMessageRow>()
    const hasMore = results.length > limit
    const pageRows = results.slice(0, limit)
    const last = pageRows.at(-1)
    return privateNaverMailJson({
...
async function ownedMessage(
  ...
  const row = await env.DB.prepare(
    `SELECT m.id, m.account_id, m.imap_uid, m.uid_validity, m.sender_name,
            ...
            m.has_attachments, a.name AS account_name, a.email AS account_email,
            a.status AS account_status
       FROM naver_mail_accounts a
       JOIN naver_mail_messages m ON m.account_id = a.id
      WHERE a.user_id = ? AND a.id = ? AND m.id = ? LIMIT 1`,
  ).bind(userId, accountId, messageId).first<NaverMailMessageRow>()
  if (!row) throw new NaverMailStoreError(404, 'NAVER 邮箱邮件不存在。')
  return row
}
...
async function remoteMessage(
  ...
  const client = await naverMailClient(account.email, account.appPassword)
  try {
    await client.open()
...
export async function getNaverMailMessage(
  env: Env,
  user: SessionUser,
  accountId: string,
  messageId: string,
  ...
  try {
    ...
    const markedRead = !row.is_read
      && await markRemoteNaverMailMessageRead(env, account, row.id, uid)
    return privateNaverMailJson({
      message: {
        ...publicMessage(row),
        ...parsed.message,
...
export async function getNaverMailAttachment(
  env: Env,
  user: SessionUser,
  accountId: string,
  messageId: string,
  ...
  try {
    ...
    if (!attachment) throw new NaverMailStoreError(404, '附件不存在。')
    const content = typeof attachment.content === 'string'
      ? new TextEncoder().encode(attachment.content)
      : attachment.content instanceof Uint8Array
        ? attachment.content : new Uint8Array(attachment.content)
    if (content.byteLength > MAX_ATTACHMENT_BYTES) {
      throw new NaverMailStoreError(413, '附件超过 5 MiB 下载上限。')
    }
