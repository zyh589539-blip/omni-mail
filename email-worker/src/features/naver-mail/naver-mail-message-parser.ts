...
import type { NaverMailMessageDetail, NaverMailMessageMetadata } from './naver-mail-types'
...
export async function parseNaverMailMetadata(
  fetchLine: string,
  headers: Uint8Array,
): Promise<NaverMailMessageMetadata> {
  const uid = Number(numericAttribute(fetchLine, 'UID'))
  if (!Number.isSafeInteger(uid) || uid < 1) {
    throw new Error('NAVER 邮箱 FETCH 响应缺少有效 UID。')
  }
...
export async function parseNaverMailMessage(
  data: Uint8Array,
  uid: string,
): Promise<{ message: NaverMailMessageDetail; parsedAttachments: Awaited<ReturnType<typeof parseImapMessage>>['parsedAttachments'] }> {
