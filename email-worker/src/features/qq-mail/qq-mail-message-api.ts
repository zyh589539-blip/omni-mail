...
import { markRemoteQqMailMessageRead } from './qq-mail-read-state'
import { QqMailAccountStore, QqMailStoreError } from './qq-mail-store'
import type { PublicQqMailAccount } from './qq-mail-types'
import type { Env, SessionUser } from '../../app/types'

const MAX_ATTACHMENT_BYTES = 5 * 1024 * 1024

type QqMailMessageRow = {
  id: string
  account_id: string
  imap_uid: number
  uid_validity: number
  ...
  account_status: PublicQqMailAccount['status']
}
...
function decodeCursor(value: string | null): { date: number; id: string } | null {
  ...
  try {
    ...
    throw new QqMailStoreError(400, 'QQ 邮箱邮件分页游标无效。')
  }
}

function publicMessage(row: QqMailMessageRow) {
  return {
    id: row.id,
    account: {
      id: row.account_id,
...
export async function listQqMailMessages(
  ...
  try {
    ...
    if (accountId && !await new QqMailAccountStore(env, user.id).publicAccount(accountId)) {
      throw new QqMailStoreError(404, 'QQ 邮箱账号不存在。')
    }
    ...
    ).bind(...bindings, limit + 1).all<QqMailMessageRow>()
    const hasMore = results.length > limit
    const pageRows = results.slice(0, limit)
    const last = pageRows.at(-1)
    return privateQqMailJson({
...
async function ownedMessage(
  ...
  const row = await env.DB.prepare(
    `SELECT m.id, m.account_id, m.imap_uid, m.uid_validity, m.sender_name,
            ...
            m.has_attachments, a.name AS account_name, a.email AS account_email,
            a.status AS account_status
       FROM qq_mail_accounts a
       JOIN qq_mail_messages m ON m.account_id = a.id
      WHERE a.user_id = ? AND a.id = ? AND m.id = ? LIMIT 1`,
  ).bind(userId, accountId, messageId).first<QqMailMessageRow>()
  if (!row) throw new QqMailStoreError(404, 'QQ 邮箱邮件不存在。')
  return row
}
...
async function remoteMessage(
  ...
  const client = await qqMailClient(account.email, account.authorizationCode)
  try {
    await client.open()
...
export async function getQqMailMessage(
  env: Env,
  user: SessionUser,
  accountId: string,
  messageId: string,
  ...
  try {
    ...
    const markedRead = !row.is_read
      && await markRemoteQqMailMessageRead(env, account, row.id, uid)
    return privateQqMailJson({
      message: {
        ...publicMessage(row),
        ...parsed.message,
...
export async function getQqMailAttachment(
  env: Env,
  user: SessionUser,
  accountId: string,
  messageId: string,
  ...
  try {
    ...
    if (!attachment) throw new QqMailStoreError(404, '附件不存在。')
    const content = typeof attachment.content === 'string'
      ? new TextEncoder().encode(attachment.content)
      : attachment.content instanceof Uint8Array
        ? attachment.content : new Uint8Array(attachment.content)
    if (content.byteLength > MAX_ATTACHMENT_BYTES) {
      throw new QqMailStoreError(413, '附件超过 5 MiB 下载上限。')
    }
