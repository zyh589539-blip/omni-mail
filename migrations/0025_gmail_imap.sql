CREATE TABLE gmail_imap_accounts (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_gmail_imap_accounts_due
  ON gmail_imap_accounts(next_sync_at, status, id);
CREATE INDEX idx_gmail_imap_accounts_user
  ON gmail_imap_accounts(user_id, created_at, id);

CREATE TRIGGER gmail_imap_accounts_limit
...
CREATE TABLE gmail_imap_messages (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_gmail_imap_messages_account_date
  ON gmail_imap_messages(account_id, internal_date DESC, id DESC);
CREATE INDEX idx_gmail_imap_messages_date
  ON gmail_imap_messages(internal_date DESC, id DESC, account_id);

CREATE TABLE gmail_imap_validation_limits (
